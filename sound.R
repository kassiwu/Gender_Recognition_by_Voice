...
# We measures 22 acoustic parameters on acoustic signals.
...
setwd("/Users/Kassi/Desktop/voice-gender")
...
voice <- rbind(males, females)
View(voice)
drops <- c("sound.files","duration","peakf")
voice <- voice[ , !(names(voice) %in% drops)]
View(voice)
dim(voice)
nrow(males)
nrow(females)
write.csv(voice, file = "~/Desktop/voice-gender/voice.csv", row.names = FALSE)
